...
const renderCell = (
  cellOverrides: Partial<CellData> = {},
  handlers?: Partial<Pick<CellProps, 'onClick' | 'onContextMenu'>>,
) => {
...
const queryIcon = (container: HTMLElement, testId: string) => {
  const fromTestId = container.querySelector(
    `[data-testid="${testId}"]`,
  ) as HTMLElement | null;
...
describe('Cell component', () => {
  ...
  it('shows a bomb icon if revealed and hasMine is true', () => {
    ...
    const bomb = queryIcon(cellEl, 'bomb-icon');
  ...
  it('shows a flag icon if cell is flagged and not revealed', () => {
    ...
    const cellEl = getByTestId('cell');
    const flag = queryIcon(cellEl, 'flag-icon');
  ...
  it('renders an exploded mine cell', () => {
    ...
    // We still expect a bomb icon for an exploded mine; styling handles "exploded"
    const bomb = queryIcon(el, 'bomb-icon');
    expect(bomb).not.toBeNull();
  });
});